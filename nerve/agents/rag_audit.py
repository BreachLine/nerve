"""RAG audit agent — vector DB access, document injection, retrieval poisoning."""
...
from nerve.agents.prompts import build_rag_audit_prompt


class RAGAuditAgent(NerveAgent):
    """Phase 2d: Audit RAG pipelines and vector databases."""
    ...
    async def run(self, task: str, **kwargs) -> AgentResult:
        ...
        intel_context = ""
        if self._intel_pool:
            intel_context = await self._intel_pool.get_context_for_agent("rag_audit")

        system_prompt = build_rag_audit_prompt(
